...
``scripts/b12x/quantize_model_fp6.py`` (or ``scripts/b12x/quantize_moe_fp6.py``), checks
...
``tests/b12x/quantization/test_fp6_moe_weights_pipeline.py``.

Example:
    python scripts/b12x/validate_fp6_moe_artifact.py --artifact out_moe/layer_0.moe_fp6.safetensors
"""
...
from flashinfer.b12x.quantization.mxfp6 import load_fp6_moe_weights
...
def _unswizzle_grid(swizzled: torch.Tensor, rows: int, num_blocks: int) -> torch.Tensor:
    ...
    """
    from flashinfer.b12x._lib.fp6 import unswizzle_mxfp6_scales
