...
Inspect the plan first (no GPU, no full-tensor loads):
    python scripts/b12x/quantize_model_fp6.py --model /path/Qwen3.6-35B-A3B --out out_moe --arch auto --dry-run

Export a full FP6 HF checkpoint (MoE):
    python scripts/b12x/quantize_model_fp6.py --model /path/Qwen3.6-35B-A3B --out qwen_fp6 --arch moe

Export a full FP6 HF checkpoint (dense, MLP + attention):
    python scripts/b12x/quantize_model_fp6.py --model /path/Qwen3.6-27B --out qwen27_fp6 --arch dense

Per-layer kernel-validation artifacts (safetensors):
    python scripts/b12x/quantize_model_fp6.py --model /path/Qwen3.6-35B-A3B --out out_moe --arch moe --format pt
"""
