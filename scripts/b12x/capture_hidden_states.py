...
Feed the result to scripts/b12x/ablate_moe_act_quant.py or
scripts/b12x/ablate_hadamard_fp6.py via ``--x-from`` so the ablation sees real
outlier-channel structure instead of gaussian noise.

Example (Qwen3.6 ships custom modeling code, hence --trust-remote-code):
    python scripts/b12x/capture_hidden_states.py \
...
def main() -> None:
    ...
    from flashinfer.b12x.quantization.mxfp6.calib_io import save_hidden_states
