...
from flashinfer.b12x.gemm import blockscaled
from flashinfer.b12x.gemm.blockscaled import _a16, _quantize
from flashinfer.b12x._lib.intrinsics import swizzle_block_scale
from flashinfer.b12x._lib.runtime_control import freeze_kernel_resolution, unfreeze_kernel_resolution
from tests.b12x._reference.helpers import require_b12x


def test_a16_functional_and_out_export_are_opaque():
    from flashinfer.b12x.gemm.blockscaled import _ops  # noqa: F401
...
def test_scale_storage_views_are_zero_copy(n, k, group):
    from flashinfer.b12x._lib.intrinsics import as_grouped_scale_view, as_grouped_scale_view_mx
...
def test_shared_checkpoint_storage_matches_cuda_weights_in_graphs(tmp_path, allocation, recipe, m):
    ...
    require_b12x()
    from flashinfer.b12x.loader import capabilities
    from benchmarks.b12x.loader._utils import WeightFiles
    caps = capabilities()
...
def test_aot_functionalization_preserves_precision_and_output(recipe, mode, use_out, backend, monkeypatch):
    ...
    from flashinfer.b12x.gemm.blockscaled import _ops
...
def test_a16_frozen_callable_and_graph_replay(recipe, monkeypatch):
    ...
    from flashinfer.b12x._lib.dense_gemm import _get_compiled_dense_gemm
...
def test_quantized_nvfp4_activation_contract():
    require_b12x()
    from flashinfer.b12x._lib.intrinsics import quantize_grouped_nvfp4_torch
    from flashinfer.b12x._lib.dense_gemm import dense_gemm
...
def test_heuristic_promotes_inside_bf16_api_before_activation_quantization(monkeypatch):
    ...
    from flashinfer.b12x.gemm.blockscaled import _policy
...
def test_native_weight_pairs_exhaustive(fp4):
    ...
    import cutlass
    import cutlass.cute as cute
    from flashinfer.b12x._lib.compiler import compile as compile_kernel
    from flashinfer.b12x._lib.intrinsics import nvfp4_pair_to_bf16x2_sm120, mxfp8_pair_to_bf16x2_sm120
    from flashinfer.b12x._lib.utils import make_ptr, current_cuda_stream
