...
def _b12x():
    return importlib.import_module("flashinfer.b12x")
...
def test_registry_matches_disk():
    ...
    assert sorted(set(b12x._OPS) - overrides) == _on_disk_ops(), (
        "flashinfer.b12x._OPS and public op directories under b12x/ must be "
    ...
    for qualname, module_path in b12x._OP_MODULE_OVERRIDES.items():
        ...
        module = importlib.import_module(f"flashinfer.b12x.{module_path}")
...
def test_namespace_and_registry_imports_stay_lightweight():
    script = """
import sys
# The enclosing FlashInfer package has its own eager dependencies. Measure
# only the additional imports introduced by the B12X namespace and registry.
import flashinfer
import torch
...
import flashinfer.b12x.attention
import flashinfer.b12x.norm
import flashinfer.b12x.sequence
import flashinfer.b12x.loader
...
apis = sorted(
    name
    for name in introduced
    if name.startswith("flashinfer.b12x.") and name.endswith(".api")
)
...
def test_every_op_meta_contract():
    ...
    for meta in b12x.list_ops():
        module = importlib.import_module(
            f"flashinfer.b12x.{b12x._op_module_path(meta.qualname)}"
        )
