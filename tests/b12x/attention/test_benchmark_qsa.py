...
from benchmarks.b12x import benchmark_qsa
...
def test_benchmark_calls_only_the_public_qsa_lifecycle() -> None:
    ...
    assert not any(
        ...
        and node.module.startswith("flashinfer.b12x.attention.qsa._")
        for node in ast.walk(tree)
    )


def test_sparse_gqa_implementation_modules_are_imported_lazily() -> None:
    repository = Path(benchmark_qsa.__file__).resolve().parents[2]
    source = textwrap.dedent(
        """
        import inspect
        import sys

        from flashinfer.b12x.attention.qsa import _sparse_gqa

        cute_module = "flashinfer.b12x.attention.qsa._sparse_gqa_cute"
        selected_paged_module = "flashinfer.b12x.attention.paged._selected_forward"
