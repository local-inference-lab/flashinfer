...
Run with torchrun on 2, 4 or 8 GPUs:
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 python -m torch.distributed.run \
        --nproc-per-node=8 tests/b12x/distributed/test_pcie_twoshot.py
"""
...
from flashinfer.b12x.comm.pcie.pcie_twoshot import (
    PCIeTwoShotSP,
    _make_layout,
    quantize_per_row,
)
...
def test_factory_retains_twoshot_ipc_ownership_when_verdict_fails(
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_twoshot._run_collective_preallocation_setup",
        fake_preallocation,
    )
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_twoshot._require_full_grid_residency",
        lambda **kwargs: None,
    )
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_twoshot._require_collective_contract",
        lambda **kwargs: None,
    )
    monkeypatch.setattr(
        PCIeOneshotAllReduce,
