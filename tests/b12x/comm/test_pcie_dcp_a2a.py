...
from flashinfer.b12x.comm.pcie import kimi_topk16
from flashinfer.b12x.comm.pcie.pcie_dcp_a2a import (
    PCIeDCPA2A,
    PCIeDCPA2APool,
    _SINGLE_CHANNEL_ID,
    _staging_layout,
...
def test_graph_epoch_uses_only_barrier_record_padding() -> None:
    from flashinfer.b12x.comm.pcie import _dcp_a2a_cute as kernels
...
def test_a2a_graph_epoch_tail_uses_serialized_stream_fast_path() -> None:
    from flashinfer.b12x.comm.pcie import _cute_intrinsics, _dcp_a2a_cute as kernels
...
def test_block_pair_barrier_selects_once_and_keeps_scaled_offsets_int64() -> None:
    from flashinfer.b12x.comm.pcie import _dcp_cute_common as common
...
def test_graph_lse_prepare_warms_shared_runtime_log_launcher(
    ...
    calls = []
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_dcp_a2a._is_current_stream_capturing",
        lambda device: False,
    )
...
def test_first_capture_freezes_the_next_eager_slot_as_graph_base(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie._dcp_a2a_cute.is_all_gather_heads_prepared",
        lambda *args: True,
    )
...
def test_kimi_pair_topk_graph_prewarm_uses_runtime_world_size(
    ...
    monkeypatch.setitem(
        sys.modules,
        "flashinfer.b12x.comm.pcie._dcp_a2a_cute",
        SimpleNamespace(
...
def test_pool_uses_distinct_channels_for_target_and_draft_captures(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_dcp_a2a._current_stream_key",
...
def test_pool_collectively_prepares_logical_channels_in_canonical_order(
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_dcp_a2a._broadcast_gather_object",
...
def test_pool_rejects_logical_channel_set_mismatch_before_allocation(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_dcp_a2a._broadcast_gather_object", gather
    )
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._broadcast_gather_object", gather
    )
...
def test_pool_coordinates_ipc_teardown_across_ranks(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_dcp_a2a.dist.barrier",
