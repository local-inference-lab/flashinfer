...
from flashinfer.b12x.comm.pcie.pcie_oneshot import (
    IPC_SLAB_ALIGNMENT,
...
from flashinfer.b12x.comm.pcie._oneshot_cute import _FusedOneshotLaunch, _OneshotLaunch
...
def test_full_grid_residency_rejects_mig_like_capacity_collectively(
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._exchange_setup_failures", exchange
    )
...
def test_prepare_plain_graph_all_reduce_retains_immutable_plan(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie._oneshot_cute.get_oneshot_launcher",
...
def test_capture_binds_unseen_shape_for_a_prepared_launcher(monkeypatch) -> None:
    monkeypatch.setenv("B12X_PCIE_TP2_PLAIN_REMOTE_PUSH", "1")
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._is_current_stream_capturing",
        lambda _device: True,
    )
    monkeypatch.setattr(
        _CuTeOneshotBackend,
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie._oneshot_cute.is_oneshot_launcher_prepared",
        lambda *args: True,
    )
    launches = []
    monkeypatch.setattr(
...
def test_runtime_rejects_reuse_from_another_stream_key(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._current_stream_key",
...
def test_autotune_never_benchmarks_past_eager_capacity(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._compute_crossover_size", fake_compute
    )
...
def test_allocate_shared_buffer_cleans_up_on_failed_peer_open(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._broadcast_gather_object",
...
def test_failed_peer_setup_export_free_is_retained_and_retryable(monkeypatch):
    ...
    monkeypatch.setattr("torch.distributed.get_rank", lambda group=None: 0)
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._broadcast_gather_object", exchange
    )
...
def test_register_graph_buffers_uses_exchange_group_broadcast(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot._object_broadcast_device",
        lambda group: "cpu",
    )
...
def test_collective_logical_channel_mismatch_rejects_before_allocation(monkeypatch):
    ...
    monkeypatch.setattr("flashinfer.b12x.comm.pcie.pcie_oneshot._broadcast_gather_object", gather)
...
def test_pool_coordinates_ipc_teardown_across_ranks(monkeypatch):
    ...
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot.dist.barrier",
...
def test_pool_binds_cuda_device_for_teardown_barriers(monkeypatch):
    ...
    pool._all_channels.append(transient)
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_oneshot.torch.cuda.synchronize",
