...
from flashinfer.b12x.comm.pcie.pcie_dcp_topk import (
    PCIeDCPTopKOwnerExchange,
    _SIGNAL_BYTES,
    _candidate_staging_layout,
    owner_stage_reference,
)
from flashinfer.b12x.comm.pcie._dcp_cute_common import signal_bytes
...
def test_graph_capture_pins_the_next_staging_slot_and_enables_prior_wait(
    ...
    capture_state = False
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie.pcie_dcp_topk._is_current_stream_capturing",
        lambda _device: capture_state,
    )
    monkeypatch.setattr(owner, "prepare_graph", lambda **kwargs: None)
    monkeypatch.setattr(
        "flashinfer.b12x.comm.pcie._dcp_topk_cute.is_topk_stage_prepared",
        lambda *args: True,
    )
