...
from flashinfer.b12x.policy import EMBEDDED_REGISTRY, DeviceIdentity
from flashinfer.b12x.policy.generation import (
    CheckpointStore,
    GenerationContext,
    GenerationSettings,
)
from flashinfer.b12x.policy.generation.moe_corpus import (
    MoeModelGeometry,
    MoeRecipe,
    expand_physical_geometries,
    expand_sweep_cases,
)
from flashinfer.b12x.policy.generation.progress import NullProgressReporter
from flashinfer.b12x.policy.generation.providers import moe_gpu_worker
from flashinfer.b12x.policy.generation.providers.moe import (
...
from flashinfer.b12x.policy.generation.reducer import (
    DecisionRecord,
    build_axis_tree,
    decision_node_to_dict,
    synthesize_integer_axis_coverage,
)
from flashinfer.b12x.policy.generation.providers.moe_gpu_worker import (
...
from flashinfer.b12x.policy.serialization import profile_from_dict
...
def test_embedded_moe_profiles_and_heuristics_cover_corpus_queries(
    ...
    from flashinfer.b12x.policy import PolicyContext, PolicySource
    from flashinfer.b12x.moe.fused_moe._policy import MOE_DECODE_POLICY, MoeDecodeQuery
...
def test_uniform_w4a8_profile_reference_models_checkpoint_semantics(
    ...
    from flashinfer.b12x.moe import fused_moe
    from flashinfer.b12x.moe._shared.kernels.reference import moe_reference_w4a8_mx
...
def test_w4a8_profile_inputs_are_mxfp8_representable() -> None:
    from flashinfer.b12x._lib.intrinsics import quant_dequant_mxfp8_torch
...
def test_independent_nvfp4_oracle_matches_kernel_scale_math(monkeypatch) -> None:
    from flashinfer.b12x.moe._shared.kernels import reference
...
def test_projection_mixed_bind_initializes_grid_barrier_workspace() -> None:
    from flashinfer.b12x.moe import fused_moe
    from flashinfer.b12x.policy import MOE_DECODE, get_auto_policy
...
def test_moe_precision_prefers_measured_parity_without_a_five_percent_margin(ratio, expected):
    from flashinfer.b12x.policy.generation.providers.moe_precision import qualifies
...
def test_precision_reduction_confirms_aggregate_without_per_route_veto(confirmed, backend):
    from flashinfer.b12x.policy.generation.providers.moe_precision import select_winner
...
def test_auto_precision_qualifies_each_capacity_and_preserves_exact_holes(tmp_path):
    from flashinfer.b12x.policy import PolicyContext, PolicyMode, PolicySource, ProfileRegistry
    from flashinfer.b12x.moe.fused_moe._policy import MOE_DECODE_POLICY, MoeDecodeQuery
...
def test_embedded_auto_precision_retains_overrides_and_exact_capacity_coverage(profile_id, route):
    ...
    from flashinfer.b12x.policy import MOE_DECODE, PolicyContext, PolicyMode, PolicySource
    from flashinfer.b12x.moe.fused_moe._policy import MOE_DECODE_POLICY, MoeDecodeQuery, MoeDecodeConfig
...
def test_auto_precision_heuristic_promotes_supported_decode(capability, capacity):
    from dataclasses import replace
    from flashinfer.b12x.policy import PolicyContext, PolicyMode, PolicySource
    from flashinfer.b12x.moe.fused_moe._policy import MOE_DECODE_POLICY, MoeDecodeQuery
