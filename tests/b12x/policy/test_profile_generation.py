...
from flashinfer.b12x.policy import EMBEDDED_REGISTRY, ComponentPolicy, DeviceIdentity
...
from flashinfer.b12x.policy.generation import parallel
...
from flashinfer.b12x.tools.generate_gpu_profile import (
...
def test_source_fingerprint_excludes_generated_profile_payloads() -> None:
    assert _is_generated_profile_data(
        Path("flashinfer/b12x/policy/_profiles/data/nvidia.gb10.48sm.json")
    )
    assert not _is_generated_profile_data(
        Path("flashinfer/b12x/policy/_profiles/data/__init__.py")
    )
