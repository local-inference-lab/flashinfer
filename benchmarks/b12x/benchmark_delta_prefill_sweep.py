...
from flashinfer.b12x.policy.generation import CheckpointStore, GenerationContext, GenerationSettings
from flashinfer.b12x.policy.generation.progress import NullProgressReporter
from flashinfer.b12x.policy.generation.providers.delta_prefill import (
    GdnPrefillGenerator, KdaPrefillGenerator, prefill_candidates, prefill_cases,
)
from benchmarks.b12x._gdn_prefill_race import _source, _versions
from benchmarks.b12x.benchmark_gdn_decode import _device_provenance, _git_provenance
from benchmarks.b12x.common import nvidia_smi_gpu_mode_snapshot, require_sm120


def source_manifest(recipe: str) -> list[dict[str, str]]:
    sources = {Path(__file__).resolve()}
    for directory in (
        ROOT / "flashinfer/b12x/sequence/_shared/delta_prefill",
        ROOT / f"flashinfer/b12x/sequence/{recipe}_prefill",
        ROOT / "flashinfer/b12x/_lib",
    ):
        sources.update(directory.glob("*.py"))
    sources.update((
        ROOT / "flashinfer/b12x/policy/generation/providers/delta_prefill.py",
        ROOT / "flashinfer/b12x/policy/generation/delta_prefill_cases.py",
        ROOT / "flashinfer/b12x/policy/generation/sweep.py",
    ))
