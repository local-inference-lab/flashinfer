...
from flashinfer.b12x.policy.generation.delta_prefill_cases import (
    GDN_PREFILL_CASES, assert_close, check_binding, make_binding, make_inputs, oracle, run_binding,
)
from benchmarks.b12x.common import make_l2_flush_fn, nvidia_smi_gpu_mode_snapshot, require_sm120
...
def policy_context_from_file(path, identity):
    ...
    from flashinfer.b12x.policy import PolicyContext, PolicyMode, ProfileRegistry
    ...
    for name, cp in factories:
        ...
        try:
            if name == "b12x":
                ...
                from benchmarks.b12x._gdn_prefill_flashinfer import FlashInferArm
...
def main(args, argv, parser):
    from benchmarks.b12x.benchmark_gdn_decode import _device_provenance, _git_provenance
    ...
    if args.policy_profile is not None:
        from flashinfer.b12x.policy.device import detect_device
    ...
    sources = [p for directory in (root/"flashinfer/b12x/sequence/_shared/delta_prefill", root/"flashinfer/b12x/sequence/gdn_prefill")
               for p in directory.glob("*.py")]
    sources += [pathlib.Path(__file__), root/"benchmarks/b12x/_gdn_prefill_flashinfer.py",
                root/"flashinfer/b12x/policy/generation/delta_prefill_cases.py"]
    provenance = {
        "command": [sys.executable, str(root/"benchmarks/b12x/benchmark_gdn_decode.py"), *argv],
