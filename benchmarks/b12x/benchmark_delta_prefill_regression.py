...
def main(argv=None):
    ...
    contract = pathlib.Path(__file__).resolve().parents[2]/"flashinfer/b12x/policy/generation/delta_prefill_cases.py"
    ...
    from benchmarks.b12x.common import nvidia_smi_gpu_mode_snapshot
    ...
    production = source/"flashinfer/b12x/sequence/kda_prefill"
    files = list(production.glob("*.py"))
    files += list((source/"flashinfer/b12x/sequence/_shared/delta_prefill").glob("*.py"))
    payload = {"provenance": {
