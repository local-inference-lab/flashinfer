...
from flashinfer.b12x.sequence import ple, ple_embedding, ple_hash
from flashinfer.b12x.sequence.ple.reference import (
    ple_projected_sequence_reference,
    ple_projected_u_reference,
)
from flashinfer.b12x.sequence.ple_embedding import reference as ple_embedding_reference
from flashinfer.b12x.sequence.ple_hash.reference import ple_hash_packed_reference
...
def _validate_hash(case: HashCase) -> dict[str, object]:
    ...
    return {
        "status": "passed",
        "oracle": "flashinfer.b12x.sequence.ple_hash.reference.ple_hash_packed_reference",
...
def _validate_embedding(case: EmbeddingCase) -> dict[str, object]:
    ...
    return {
        "status": "passed",
        "oracle": "flashinfer.b12x.sequence.ple_embedding.reference.fused",
        "output": _finite_nonzero_close(
            case.binding.out, case.expected, rtol=0.0, atol=0.0
        ),
    }
...
def _validate_layer(case: LayerCase) -> dict[str, object]:
    ...
    return {
        "status": "passed",
        "oracle": "flashinfer.b12x.sequence.ple.reference",
        "output": _finite_nonzero_close(
...
def _validate_device(device: torch.device) -> None:
    ...
    for name, module in (
        ...
        if not module.is_supported(device):
            raise SystemExit(f"flashinfer.b12x.sequence.{name} is unsupported on {device}")
