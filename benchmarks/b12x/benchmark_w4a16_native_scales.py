...
from flashinfer.b12x._lib.compiler import clear_compile_cache
from flashinfer.b12x.moe import fused_moe
from flashinfer.b12x.moe.fused_moe import _impl
from flashinfer.b12x.policy import MOE_DECODE, get_auto_policy
from benchmarks.b12x.benchmark_blockscaled_precision import _clock_checks, _paired, _snapshot
from benchmarks.b12x.benchmark_moe import get_quant_mode_params
from benchmarks.b12x.benchmark_w4a16_nvfp4_layouts import _check
from benchmarks.b12x.common import make_l2_flush_fn
from benchmarks.b12x.moe_checkpoint_snapshot import load_snapshot, tensor_digest
from tests.b12x._reference.w4a16_reference import moe_reference_w4a16


@contextmanager
def baseline_modules(revision, directory):
    import flashinfer.b12x.moe._shared.kernels.w4a16 as package
    saved = {}
    hashes = {}
    try:
        for name in ("prepare", "kernel"):
            ...
            source = subprocess.check_output([
                "git", "show", f"{revision}:flashinfer/b12x/moe/_shared/kernels/w4a16/{name}.py",
            ], cwd=ROOT)
...
def main():
    ...
    report = dict(command=[sys.executable, *sys.argv], tensor_digests=digests,
        ...
        source_sha256={name: hashlib.sha256((ROOT / f"flashinfer/b12x/moe/_shared/kernels/w4a16/{name}.py").read_bytes()).hexdigest()
