...
from flashinfer.b12x.norm import hyperconnection as hc
...
def _correctness_gate(case: _Case, operator: str) -> dict[str, object]:
    ...
    return {
        "status": "passed",
        "oracle": "flashinfer.b12x.norm.hyperconnection.reference",
...
def _validate_device(device: torch.device) -> None:
    if not hc.is_supported(device):
        raise SystemExit(
            "flashinfer.b12x.norm.hyperconnection reports that its runtime kernels are "
    ...
    emitter.emit(
        {
            ...
            "timed_path": "flashinfer.b12x.norm.hyperconnection public runtime API",
            "fallback_path": "none",
