...
Launch with torchrun on every node (one GPU per node)::
    ...
    torchrun --nnodes=4 --nproc-per-node=1 --node-rank=$RANK \\
        ...
        benchmarks/b12x/benchmark_roce_oneshot.py --output docs/b12x/evidence/rocenante/<receipt>.json
...
def main() -> None:
    ...
    from flashinfer.b12x.comm import roce
    ...
    if rank == 0:
        doc = {
            "schema": "flashinfer.b12x.comm.roce.oneshot.benchmark",
            "version": 3,
