...
PERFORMANT, not merely correct -- the payoff of PTX parity. Three contracts:
  ...
  * DSV4 decode  : legacy compressed_sparse_mla_decode_forward (flag OFF)
                   ...
                   benchmarks/b12x/benchmark_compressed_sparse_mla.py.
  * GLM  decode  : legacy sparse_mla_decode_forward (flag OFF)
                   vs unified run_unified_decode (flag ON), q_head_dim=576,
                   656B/token KV.  Shapes mirror benchmarks/b12x/benchmark_mla.py
...
from benchmarks.b12x.common import (
    bench_cuda_graph,
    capture_cuda_graph,
    make_l2_flush_fn,
    require_sm120,
...
from flashinfer.b12x.attention._shared.mla.reference import pack_mla_kv_cache_reference
from flashinfer.b12x.attention._shared.mla.api import clear_mla_caches, sparse_mla_decode_forward
...
class UnifiedSpy:
    ...
    def install(self) -> None:
        import flashinfer.b12x._lib.compiler as compiler_mod
        import flashinfer.b12x.attention._shared.mla.kernel as launch_mod
        import flashinfer.b12x.attention._shared.mla.prefill as prefill_mod
...
def _time_graph(run, *, spy: UnifiedSpy, warmup: int, replays: int, l2_flush):
    import flashinfer.b12x.attention._shared.mla.kernel as _launch_mod
...
def _import_prefill_ref():
    from tests.b12x._reference import dsv4_ref, prefill_ref

    return prefill_ref, dsv4_ref


def _repack_prefill_to_compressed(packed_dsv4, page_size, num_blocks, dsv4_ref):
    import flashinfer.b12x.attention._shared.mla.compressed_reference as cr
...
def bench_dsv4_prefill(
    *, num_tokens, num_heads, topk, device, spy, warmup, replays, l2_flush, seed
    ...
    from flashinfer.b12x.attention._shared.mla.prefill import (
