...
from flashinfer.b12x.gemm.blockscaled import _a16
from flashinfer.b12x._lib.dense_gemm import dense_gemm, dense_gemm_fused_quant_a
...
from flashinfer.b12x.gemm._shared.wo_mxfp8 import quantize_mxfp8_rows_torch, dequantize_mxfp8_rows_torch
from benchmarks.b12x.common import make_l2_flush_fn
...
def _compile_state():
    from flashinfer.b12x._lib.compiler import compile_cache_info
...
def run(args, specs, *, flashinfer_error):
    ...
    paths = [*sorted((root / "flashinfer/b12x/gemm/blockscaled").glob("*.py")),
             root / "flashinfer/b12x/_lib/dense_gemm.py", root / "flashinfer/b12x/_lib/intrinsics.py", pathlib.Path(__file__).resolve()]
    ...
    with args.evidence.open("x") as evidence:
        ...
        for name, k, n, _ in specs:
            ...
            for m in counts:
                ...
                outputs = [qout]
                if recipe == "mxfp8":
                    from flashinfer.b12x.gemm.blockscaled._linear import _packed_mxfp8_op
                    def existing():
