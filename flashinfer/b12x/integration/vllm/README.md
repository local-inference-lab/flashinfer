...
``flashinfer/b12x/integration/`` and calls the public ``plan`` / ``bind`` / ``run``
...
### 1. Install the FlashInfer fork

```bash
python -m pip install -e /path/to/flashinfer
```

### 2. Plugin discovery

The fork's ``flashinfer-python`` distribution already ships the entry point
below. Do not add a duplicate registration to vLLM or install a separate b12x
distribution. The automatic ``b12x`` namespace forwards legacy imports to
``flashinfer.b12x``.

```toml
[project.entry-points."vllm.general_plugins"]
b12x_fp6 = "flashinfer.b12x.integration.vllm.plugin:register_b12x_fp6"
```
...
See [the FP6 vLLM integration guide](../../../../docs/b12x/mxfp6-vllm-integration.md) for
