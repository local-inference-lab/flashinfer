...
from flashinfer.b12x.gemm.blockscaled._policy import A16_CONFIGS, BLOCKSCALED_PRECISION
from flashinfer.b12x.policy.generation.reducer import DecisionRecord, build_axis_tree
from flashinfer.b12x.policy.generation.sweep import DiscreteSweepGenerator, SweepCandidate, SweepCase, SweepMeasurement
from flashinfer.b12x.policy.types import FrozenMapping
...
def _source_identity():
    ...
    paths = (root / "_lib/dense_gemm.py", root / "_lib/intrinsics.py", Path(__file__),
             *sorted((root / "gemm/blockscaled").glob("*.py")),
             root.parent.parent / "benchmarks/b12x/benchmark_blockscaled_precision.py")
    payload = {str(p.relative_to(root.parent.parent)): hashlib.sha256(p.read_bytes()).hexdigest()
               for p in paths}
...
def qualifies(pairs, candidate, baseline):
    from benchmarks.b12x.benchmark_blockscaled_precision import _ratio_interval
...
class _Session(AbstractContextManager):
    ...
    def measure(self, case, candidates):
        ...
        from benchmarks.b12x.benchmark_blockscaled_precision import (
