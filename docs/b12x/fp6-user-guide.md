...
Snapshot installation: use the single `flashinfer-python` distribution from
this FlashInfer checkout, as described in [the snapshot README](README.md).
The implementation is `flashinfer.b12x`; legacy `b12x` imports and the
`b12x_fp6` vLLM method name remain available without a second distribution.
Commands below run from the FlashInfer repository root. Recorded results
and the older serving/scoring environment pins are historical evidence,
not fresh qualification of this relocated snapshot.
...
```bash
python scripts/b12x/quantize_model_fp6.py \
...
```bash
python scripts/b12x/quantize_model_fp6.py --model <dir> --out /tmp/x --arch auto --dry-run
```
...
The historical vLLM environment below pins its own torch. Its recorded
`torch==2.11.0` is below this snapshot's declared B12X requirement
`torch>=2.12`; do not treat the example as a compatible default installation.
For a new environment, install this checkout's `flashinfer-python` with its
declared dependencies and use a compatible vLLM build. To reproduce the
historical environment in isolation, the original `--no-deps` procedure is
retained below; it requires separately provisioning all dependencies,
including CUTLASS DSL 4.6.2, and validating that exact stack:
...
# 3. flashinfer-python (includes B12X), historical pinned-stack reproduction:
uv pip install -e /path/to/flashinfer --no-deps

python -c "import torchvision, vllm; from flashinfer import b12x; print('stack OK')"
```

For that historical reproduction, a torch upgrade changes the recorded
environment and requires a new qualification.

Installing this checkout's `flashinfer-python` registers the vLLM plugin via the
`vllm.general_plugins` entry point
(`flashinfer.b12x.integration.vllm.plugin:register_b12x_fp6`); it runs in
...
#   (flashinfer.b12x.integration.vllm.plugin:register_b12x_fp6) registers the
...
# The vLLM plugin (flashinfer.b12x.integration.vllm.plugin) reads the FP6 checkpoint
...
# PROFILE_DIR; summarize with b12x scripts/b12x/summarize_vllm_trace.py.
PROFILE="${PROFILE:-0}"
...
#   (flashinfer.b12x.integration.vllm.plugin, wired via the vllm.general_plugins
