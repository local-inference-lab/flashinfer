...
`flashinfer.b12x.comm.roce` (RoCEnante) is an all-reduce and all-gather runtime for tensor
...
`docs/b12x/evidence/rocenante/20260903-4spark-bf16-standalone-00280f2.json` (emitted by
`benchmarks/b12x/benchmark_roce_oneshot.py`) holds the command, source revision,
...
`docs/b12x/evidence/rocenante/serving-ab-final-20260903/` with commands, revisions,
...
for the same image and configuration (`docs/b12x/evidence/rocenante/serving-ab-20260902/`: per-arm decode and prefill
...
`tests/b12x/comm/test_roce_oneshot_gpu.py` (torchrun, 2+ nodes): NCCL parity,
...
and graph mode checking the fail-stop contract on every rank. `benchmarks/b12x/benchmark_roce_oneshot.py`
