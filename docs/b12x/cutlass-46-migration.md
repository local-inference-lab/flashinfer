...
`validation/b12x/cutlass_migration/data/cute_production_kernel_coverage.txt` prevents a symmetric but empty
...
python -m validation.cutlass_migration evidence resources "$CACHE_45" \
  ...
  --require-kernel-id-pattern-file validation/b12x/cutlass_migration/data/cute_production_kernel_coverage.txt \
  --require-kernel-symbol-pattern-file validation/b12x/cutlass_migration/data/cute_kernel_symbol_coverage.txt
...
# inventory in docs/b12x/cutlass-46-register-accounting.csv.
python -m validation.cutlass_migration evidence register-accounting "$DELTA_REPORT" \
  ...
  --output docs/b12x/cutlass-46-register-accounting.csv
...
The JSON uses schema `flashinfer.b12x.cute.migration.release_artifact_index.v1`; the CSV
uses `flashinfer.b12x.cute.migration.release_exception_index.v1`. The command imports no
...
`validation/b12x/cutlass_migration/data/cute_migration_abba_producer_contracts.json`.
Audit it offline
...
`docs/b12x/cutlass-46-w4a16-prefill-performance.csv`; the full reports are diagnostic
...
`docs/b12x/cutlass-46-resource-annotations.csv`, keyed by the exact
...
closeout: `docs/b12x/cutlass-46-register-accounting.csv` remains the complete alarm
