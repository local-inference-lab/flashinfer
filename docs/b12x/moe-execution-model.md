...
`MoEExecutionPlan` in `flashinfer.b12x.moe.execution` name those choices independently.
...
(``docs/b12x/btx-checkpoint-format.md``), which stores a fixed-rate trellis
...
`benchmarks/b12x/benchmark_w4a16_nvfp4_layouts.py` compares both A16 layouts through
...
are not exported from `flashinfer.b12x.integration`.
