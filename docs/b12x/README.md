# B12X snapshot in FlashInfer

`flashinfer.b12x` is an SM120/SM121 CuTe DSL and Triton kernel library for local LLM inference.
...
Run this command from the root of the `local-inference-lab/flashinfer` fork
(or use `python -m pip install -e .` for development). The upstream PyPI
release does not contain this snapshot.
The single `flashinfer-python` distribution ships both the canonical
`flashinfer.b12x` implementation and the automatic legacy `b12x` import
namespace; do not install a separate `b12x` distribution. This is a namespace
relocation, not integration into FlashInfer's other APIs.

Run the commands in these documents from the FlashInfer repository root.
Support files live under `tests/b12x`, `benchmarks/b12x`, `scripts/b12x`,
`docs/b12x`, `skills/b12x`, and `validation/b12x`. The original root
configuration is preserved in `docs/b12x/source` as historical material,
including the original `pyproject.toml`; it is not an active second build.
The original license is retained at `LICENSE.b12x.txt`.

Historical evidence retains its original revisions, machine paths, hashes,
and measurements; it does not certify this snapshot. The old
`validation.cutlass_migration` Python sources and `tests/_legacy` directory
were absent from the tracked source snapshot. Tests that require the missing
validation helpers remain intact and require that original evidence
infrastructure before they can run. Historical PCIe control-node tooling
still requires the old `.cu` extension implementation it was designed to
measure; the current CuTe-only runtime does not supply that implementation.
...
Every kernel is one op at `flashinfer.b12x.<group>.<op>`; `list_ops()` enumerates the
...
`<group>/_shared/` and the universal compile/scratch spine in `flashinfer/b12x/_lib/`.
...
see [GDN prefill](gdn-prefill.md)), `sequence.kda_prefill` (KDA prefill),
...
RDMA all-reduce/all-gather for multi-node DGX Spark TP, see `docs/b12x/rocenante.md`),
...
Every stateful kernel lives at `flashinfer.b12x.<group>.<op>` and shares the **same
...
from flashinfer.b12x.attention import compressed_sparse_mla
...
collectives are stateful classes. `flashinfer.b12x.list_ops()` enumerates the full
...
- `tests/b12x/` is the executable spec — per-group API and numerical-reference
  tests showing exact tensor layouts and `plan`/`bind`/`run` call sequences.
  The former `tests/_legacy/` reference is historical; that directory was
  not present in the source snapshot.
- `benchmarks/b12x/` has tuned invocations per kernel family (and `probe_*` scripts
  from tile-sweep experiments).
- `docs/b12x/` has design notes: the MoE execution model, the eager-plan-bind
