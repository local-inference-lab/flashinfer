...
`flashinfer/b12x/_lib/dense_gemm.py::DenseGemmKernel`. The specialization retains the dense
...
patterns in `flashinfer/b12x/moe/_shared/kernels/w4a16/kernel.py` and the native FP8
conversion helpers in `flashinfer/b12x/_lib/intrinsics.py`. The MoE engine and its prepared
...
`flashinfer/b12x/gemm/blockscaled/_quantize.py`.
...
```bash
.venv/bin/python scripts/b12x/generate_gpu_profile.py \
...
With the embedded profile loaded, `tests/b12x/gemm/test_blockscaled_a16.py`,
`tests/b12x/policy/test_blockscaled_precision.py`, and
`tests/b12x/policy/test_discrete_sweep_generator.py` passed 97 checks; four GB10-only
...
/home/luke/projects/vllm/.venv/bin/python scripts/b12x/generate_gpu_profile.py \
  --components gemm.blockscaled_precision --warmup 25 \
...
```bash
CUTE_DSL_ARCH=sm_121a .venv/bin/python benchmarks/b12x/benchmark_dense_gemm.py \
