...
`flashinfer.b12x.loader.read_tensor` is the earlier allocation-qualification primitive.
Its buffered raw-file reader is separate from the adapter's O_DIRECT transport.

Add bounded checkpoint loading to flashinfer.b12x. The component owns checkpoint manifests,
...
flashinfer/b12x/integration/vllm/
    loader.py           public vLLM loader registration and adapters
```

The loader imports without vLLM. Importing `b12x` or `flashinfer.b12x.loader` must not build
...
Add behavior tests under `tests/b12x/loader/` and startup/transport benchmarks under
`benchmarks/b12x/loader/`. The existing `benchmarks/b12x/checkpoint_loader.py` is a small
...
is in the companion vLLM checkout's `docs/b12x/design/streaming_weight_loading.md`.
