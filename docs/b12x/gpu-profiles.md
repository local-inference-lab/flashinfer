...
from flashinfer.b12x.policy import get_auto_policy
...
`flashinfer.b12x.policy.list_planning_components()` is the authoritative inventory for
...
`benchmarks/b12x/benchmark_moe.py` and the attention benchmark suite. GLM-5.2
...
`benchmarks/b12x/benchmark_moe_precision_policy.py` runs this registered provider for
...
`flashinfer/b12x/policy/_profiles/data/` is gzip-compressed and contains only the validated
