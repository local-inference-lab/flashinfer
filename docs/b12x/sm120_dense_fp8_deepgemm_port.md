...
| file | change |
|---|---|
| `flashinfer/b12x/gemm/dense.py` | `_select_default_mma_tiler_mn`: `(64,128)` default + `expected_m` regime tiles; `dense_gemm(..., expected_m=None)` |
| `flashinfer/b12x/gemm/block_fp8_linear.py` | `expected_m` through `block_fp8_linear_mxfp8` / binding / build / workspace+scratch `.bind` / `prewarm` |
| `flashinfer/b12x/gemm/wo_projection.py` | `expected_m` through wo_a/wo_b leaves / both orchestrators / both bindings / both builds / all four `.bind*` |
| `benchmarks/b12x/probe_dense_fp8_tile_sweep.py` | new: per-(M,tile) sweep, parameterized by N,K (the evidence behind the tile choice) |
| `tests/b12x/test_dense_gemm_expected_m.py` | new: regime-selection unit tests |
| `tests/b12x/test_gemm_block_fp8_linear.py`, `tests/b12x/test_gemm_wo_projection.py`, `tests/b12x/test_wo_projection_scratch_bindings.py` | hint integration + fake updates |
...
~710). Tile sweep (`benchmarks/b12x/probe_dense_fp8_tile_sweep.py`, Nemotron
...
HEAD vs pre-graft baseline `3083e936`** (`benchmarks/b12x/probe_wo_decode_latency.py`):
...
- **Re-derive the tile** with `benchmarks/b12x/probe_dense_fp8_tile_sweep.py [N K]`
...
- `quantize_block_fp8_linear_input_mxfp8` is **byte-for-byte identical** to
  ...
  UE8M0 scales (`benchmarks/b12x/probe_quant_parity_deepgemm.py`: 100%/100%).
- The `ceil(log2)/exp2` UE8M0 path equals DeepGEMM's bit-exact `ceil_to_ue8m0`
  ...
  (`benchmarks/b12x/probe_quant_parity_adversarial.py`: 0 mismatches).
...
- DeepSeek checkpoints carry `(w_fp8, weight_scale_inv)` where `weight_scale_inv
  ...
  (`vllm-other …/scaled_mm/flashinfer.b12x.py`) feeds it straight to
...
- Measured (`benchmarks/b12x/probe_weight_requant_parity.py`, N=K=4096): weight
...
**Fix** (`flashinfer/b12x/gemm/wo_projection.py`): when the block scale is **not** already
...
- **Gate:** `tests/b12x/test_fp8_quant_deepgemm_parity.py` (new) — the byte-exact
