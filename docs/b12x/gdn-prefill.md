...
`flashinfer.b12x.sequence.gdn_prefill` computes the scalar-gated delta-rule recurrence over
...
`flashinfer.b12x.sequence.gdn_decode`.
...
`flashinfer/b12x/sequence/_shared/delta_prefill/`.
...
`tests/b12x/sequence/test_gdn_prefill.py` covers the sequential oracle, strong decay
...
```bash
.venv/bin/python benchmarks/b12x/benchmark_gdn_decode.py \
...
`FlashInfer_us / b12x_us`; larger than one favors flashinfer.b12x. JSON files are never
overwritten.
...
`benchmarks/b12x/benchmark_delta_prefill_regression.py --source-root TREE --json FILE`
